...
class Decoration(BuilderExtension):
	def __init__(self):
		super().__init__('decoration.glade', ['box_main'])
		...
		self.frame_top.connect('draw', self.draw_frame_top)
		self.frame_top_right.connect('draw', self.draw_frame_top_right)
		self.frame_right.connect('draw', self.draw_frame_right)
		self.frame_bottom_right.connect('draw', self.draw_frame_bottom_right)
		self.frame_bottom.connect('draw', self.draw_frame_bottom)
		self.frame_bottom_left.connect('draw', self.draw_frame_bottom_left)
		self.frame_left.connect('draw', self.draw_frame_left)
		self.frame_top_left.connect('draw', self.draw_frame_top_left)
		
		self.box_top.set_app_paintable(True)
		self.box_top.connect('draw', self.draw_box_top)
		
		self.drawingarea_icon.connect('draw', self.draw_icon)
	
	def draw_icon(self, widget, ctx):
		ctx.set_source_rgba(1, 1, 1, 1)
		ctx.paint()
	
	def draw_box_top(self, widget, ctx):
		ctx.set_source_rgba(*self.background_color)
		ctx.paint()
	
	def draw_frame_top(self, widget, ctx):
		ctx.set_source_rgba(*self.background_color)
		ctx.paint()
	
	def draw_frame_top_right(self, widget, ctx):
		ctx.set_source_rgba(*self.background_color)
		ctx.paint()
	
	def draw_frame_right(self, widget, ctx):
		ctx.set_source_rgba(*self.background_color)
		ctx.paint()
	
	def draw_frame_bottom_right(self, widget, ctx):
		ctx.set_source_rgba(*self.background_color)
		ctx.paint()
	
	def draw_frame_bottom(self, widget, ctx):
		ctx.set_source_rgba(*self.background_color)
		ctx.paint()
	
	def draw_frame_bottom_left(self, widget, ctx):
		ctx.set_source_rgba(*self.background_color)
		ctx.paint()
	
	def draw_frame_left(self, widget, ctx):
		ctx.set_source_rgba(*self.background_color)
		ctx.paint()
	
	def draw_frame_top_left(self, widget, ctx):
		ctx.set_source_rgba(*self.background_color)
		ctx.paint()
...
class Desktop:
	def __init__(self):
		...
		self.main_window.set_startup_id('--desktop') # TODO: set desktop role
		#print(self.main_window.get_window())
		#self.main_window.connect('screen-changed', self.__screen_changed)
		#self.main_window.connect('draw', self.__expose_draw)
		#self.main_window.connect('realize', self.__realized)
		#self.main_window.set_app_paintable(True)
		self.main_window.set_can_focus(True)
		#self.__screen_changed(self.main_window, None)
	...
	def __realized(self, window):
		wayland_window = window.get_window()
		print()
		print(wayland_window)
		print(wayland_window.list_properties())
		#print(wayland_window.get_property('startup-id'))
		#wayland_window.set_startup_id('--desktop') # TODO: set desktop role
	...
	def create_window(self, window):
		...
		decoration.frame_main.add(window)
		window.decoration = decoration
		#self.main_area.add_named(decoration.main_widget, str(time()))
		self.main_area.pack_start(decoration.main_widget, False, True, 4)
	...
	def map_window(self, window):
		window.decoration.main_widget.show()
	
	def unmap_window(self, window):
		window.decoration.main_widget.hide()
...
class Manager(GObject.Object):
	...
	def create_window(self, window_id, desktop_id):
		...
		c = window.connect('size-allocate', lambda _window, rect: self.emit('set-geometry', window_id, rect.x, rect.y, rect.width, rect.height))
		...
		window.__signals = a, b, c, d, e, f, g, h, i, j
